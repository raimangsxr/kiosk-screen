...
- Operators can configure kiosk name, durations, animation defaults, inline ad count, sponsor-strip item border (radius, width, color), video-end delay, **novelty max defer transitions** (`noveltyMaxDeferTransitions`, default 3, range 1–10), and enabled state. `remoteControlPollingSeconds` is deprecated (CHG-041); SSE reconnect policy replaces polling cadence configuration.
