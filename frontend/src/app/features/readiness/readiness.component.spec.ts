...
describe('ReadinessFacade', () => {
  ...
  it('refresh sets ready when report is ready', () => {
    ...
    expect(facade.blocked()).toBeFalse();
  });

  it('refresh maps backend envelope errors', () => {
...
describe('ReadinessComponent (Material)', () => {
  ...
  it('renders ready state when report says ready', () => {
    ...
    expect(text).toContain('All required setup items are present');
  });

  it('renders warnings list when present', () => {
    fixture.detectChanges();
    httpController.expectOne('/api/readiness').flush(buildReport({ ready: true, blockers: [], warnings: ['Low content variety'] }));
    fixture.detectChanges();
    const text = fixture.nativeElement.textContent as string;
    expect(text).toContain('Low content variety');
    expect(text).toContain('Review');
  });

  it('exposes a redacted error message when readiness fails', () => {
    fixture.detectChanges();
    httpController.expectOne('/api/readiness').flush(
      { code: 'unexpected_error', message: 'Failure at /var/log/readiness.log', category: 'unexpected' },
      { status: 500, statusText: 'Server Error' }
    );
    fixture.detectChanges();
    const text = fixture.nativeElement.textContent as string;
    expect(text).toContain('Readiness unavailable');
