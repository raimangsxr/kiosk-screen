...
describe('DisplayConfigFacade', () => {
  ...
  it('save exposes backend error through error signal', () => {
    facade.save({ ...configuration, name: 'Bad' }).subscribe({ error: () => undefined });
    httpController.expectOne('/api/display/configuration').flush(
      { code: 'validation_failed', message: 'Bad values', category: 'validation' },
      { status: 422, statusText: 'Unprocessable Entity' }
    );
    expect(facade.error()?.category).toBe('validation');
  });
...
describe('DisplayConfigComponent (Reactive Forms + Material)', () => {
  ...
  it('populates the form with loaded configuration', () => {
    ...
    expect(form.controls.defaultAdRotationAnimation.value).toBe('slide');
  });
  ...
  it('saves a valid configuration', () => {
    ...
    expect(fixture.componentInstance['form']!.controls.name.value).toBe('Renamed kiosk');
  });

  it('rejects inline ad count of zero (positiveInteger)', () => {
    const form = fixture.componentInstance['form']!;
  ...
  it('rejects top duration of zero', () => {
    const form = fixture.componentInstance['form']!;
    form.controls.defaultTopDurationSeconds.setValue(0);
    expect(form.controls.defaultTopDurationSeconds.hasError('positiveInteger')).toBeTrue();
    fixture.componentInstance.submit();
    httpController.expectNone((req) => req.method === 'PUT');
  });

  it('exposes a save error when backend rejects the payload', () => {
    const form = fixture.componentInstance['form']!;
    form.controls.name.setValue('Bad');
    fixture.componentInstance.submit();
    httpController.expectOne('/api/display/configuration').flush(
      { code: 'validation_failed', message: 'Bad values', category: 'validation' },
      { status: 422, statusText: 'Unprocessable Entity' }
    );
    expect(fixture.componentInstance['saveError']()?.category).toBe('validation');
  });
  ...
  it('reports not dirty while saving', () => {
    const form = fixture.componentInstance['form']!;
    form.controls.name.setValue('Edited');
    TestBed.inject(DisplayConfigFacade)['savingState'].set(true);
    expect(fixture.componentInstance.hasUnsavedChanges()).toBeFalse();
  });
});
